...
class VideoTrainer(object):
    ...
    def train(self):
        ...
        pred = []
        target = []
        for video, label in (tqdm(self.train_loader, ascii=True, ncols=60)):
            # reset gradients
            self.optimizer.zero_grad()
            ...
            prob_label = self.net(video)
            predicted_label = torch.ge(prob_label, 0.5).float()
            prob_label = prob_label.squeeze(1)
            label = label.float()
            # print("\n\n")
            # print(label.shape)
    ...
    def test(self):
        ...
        pred = []
        prob = []
        for video, label in tqdm(self.test_loader, ascii=True, ncols = 60):
            video = video.cuda()
            label = label.cuda()
